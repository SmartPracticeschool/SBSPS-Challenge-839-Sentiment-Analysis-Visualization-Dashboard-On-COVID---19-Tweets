...
def getTweets():
    ...
    startTime = datetime.datetime(currentTimeStamp.year, currentTimeStamp.month, startTimeStampDay,
                                  startTimeStampHour, currentTimeStamp.minute, currentTimeStamp.second)
    endTime = datetime.datetime(currentTimeStamp.year, currentTimeStamp.month, currentTimeStamp.day,
                                currentTimeStampHour, currentTimeStamp.minute, currentTimeStamp.second)
    ...
    for tweet in tweepy.Cursor(auth_api.search, q="#lockdown", count=200,
        ...
        if tweet.created_at < endTime and tweet.created_at > startTime:
            ...
            prediction = modal.classify(
                dict([token, True] for token in customTokens))
            ...
            posts["results"][str(startTimeStampHour)+"-" +
                             str(currentTimeStampHour)].append(data)
